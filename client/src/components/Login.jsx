...
const Login = () => {
  ...
  setUser để cập nhật thông tin user sau login, và setCartItems để cập nhật giỏ hàng.*/
    ...
    const onSubmitHandler = async (event) => { // Ngăn chặn hành vi mặc định của biểu mẫu (tải lại trang)
      ...
      try {
        ...
        if (data.success) {
          ...
          if (state === 'login') {
            // Backend trả về: { success, message, data: { role, name, id, email, cartData } }
            // KHÔNG CÓ user key, data.data chính là user object
            if (data.data) {
              const userData = data.data; // data.data chính là user object
              console.log("=== SETTING USER TO CONTEXT ===");
              console.log("User data:", userData);
              
              // Lưu vào localStorage để persist sau khi reload
