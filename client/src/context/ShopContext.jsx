...
const ShopContextProvider = ({ children }) => {
  ...
  }); // State lưu trữ thông tin người dùng đã đăng nhập
  ...
  const fetchUser = async () => {
    // ƯU TIÊN localStorage - không xóa user nếu đã có trong localStorage
    const savedUser = localStorage.getItem('user');
    if (savedUser) {
      try {
        ...
        return; // Dừng lại, không gọi API nếu đã có user trong localStorage
      } catch (error) {
    ...
    // Chỉ gọi API nếu KHÔNG có user trong localStorage
    try {
  ...
  const value = {
    ...
    categories,
    user,
    setUser,  // Export setUser để Login component có thể set user trực tiếp
    isAdmin,
    setIsAdmin,
    cartItems,
    setCartItems,
